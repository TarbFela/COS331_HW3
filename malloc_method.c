...
int Total_Hits = 0 ;
int Local_Hits_Arr[Num_Threads];
int lock = 0 ;
...
int main()
{
    ...
    int i ;
    double pi ;
    int my_args[Num_Threads];
    for(i =0; i<Num_Threads;i++) {
        Local_Hits_Arr[i] = 0; // just to be safe...
        my_args[i] = i;
    }
    ...
    for (i = 0; i < Num_Threads ; i++)
    {
        int *my_arg = &my_args[i];
        ret = pthread_create(&ThreadID[i], NULL, Compute, (void *)my_arg) ;
    }
    Total_Hits = 0;
    for(i = 0 ; i < Num_Threads ; i++) {
        pthread_join(ThreadID[i], NULL);
        Total_Hits += Local_Hits_Arr[i];
    }
